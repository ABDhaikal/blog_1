...
const useGetBlogs = (queries: getBlogQuery) => {
   return useQuery({
      ...
      queryFn: async () => {
         ...
         let counturl = `${BASE_BLOG_URL}/count?pageSize=${display}&offset=${offset}&where=%60title%60%20LIKE%20'%25${search}%25'`;
         if (category !== "all") {
            baseurl += `%20AND%20%60category%60.%60name%60%20%3D%20'${category}'`;
            counturl += `%20AND%20%60category%60.%60name%60%20%3D%20'${category}'`;
         }
         baseurl +=
            "&sortBy=%60created%60%20desc&loadRelations=category%2Cuser";
